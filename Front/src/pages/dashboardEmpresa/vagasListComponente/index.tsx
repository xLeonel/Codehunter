        ...
        <div>
            ...
            <table width="100%" style={{ textAlign: "center" }}>
                ...
                {vagas.map((vaga: any, index) => {
                    let dataInicio = vaga.dataPostada;
                    let dataFormatada = dataInicio.replace(/(\d*)-(\d*)-(\d*).*/, '$3/$2/$1');

                    return (
                        <tr key={index}>
                            <td>{vaga.titulo}</td>
                            <td>{vaga.idAreaAtuacaoNavigation.nomeAreaAtuacao}</td>
                            <td>{dataFormatada}</td>
                            <td>R$ {vaga.idRegimeContratacaoNavigation.expectativaSalario}</td>
                            <td><IconButton onClick={e => {
                                handleClick(e);
                                setTitulo(vaga.titulo);
                                setAreaAtuacao(vaga.idAreaAtuacaoNavigation.nomeAreaAtuacao);
                                setCidade(dataFormatada);
                                setEmpresa(vaga.idRegimeContratacaoNavigation.expectativaSalario);
